Help me break down this quesion of research in the following way:\
   1. Add code comments to each and every python and shell file:\
     1a. Wherever the cyclomatic complexity may become greater than n*log(n) propose an algorithm or strategy that may reduce the number of processing cycles and/or reduce the size and space in terms of memory \
     1b. Wherever a function, class, or type is defined and unreferenced, try and understand what its intended purpose was for and where it might best be used.
     1c. Flag for critical review any function or test which makes use of random numbers, or hardcoded values, which should be limited to the constraints of the data collection methodologies.
     1d. Implement and design recommendations for a deterministic model: meaning that we have been given an unpredictable input, by which the velocity of the AUC scoring methodology are improving over time in an accelerated manner.
   2. Generate code wherever there lacks the implementation of the learning capability, training methodology, which are proposed below.\
   3. Generate experiments and tests which can be measured in terms of performance, complexity, and accuracy
   4. Follow past precidence along which will be suitable for submission to Kaggle, RSNA, and 
   5. Within the code comments, itemize tasks with @cursor label. Within new code that is generated, explain in simple and consice terms what the function, data type, class accomplishes, and where else it is used. Explain inputs and outputs
